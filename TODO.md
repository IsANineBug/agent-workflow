...
#### T1.1 · 复验 G-A（§1「用户两个选项都不选」） —— ⚠️ 第一轮**部分失败**，新发现 G-O，已修，待复跑

第一轮结果（2026-09-19，详见 `ACCEPTANCE.md` 复验节）：agent 说对了「skill 不适用」，没再「就地补规格」，
但把「你看着办吧」当成 skill 外开工的授权 —— 新建 `index.html` 并覆盖了 `src/main.js` 桩。
**新缺口 G-O**：「停手」的宾语不明（停 skill ≠ 停任务）。已在 §1 补「停手停的是整个任务」并同步安装点，
**还需用干净场地复跑一轮确认**。
