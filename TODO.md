...
> 交接文档，更新于 2026-09-19（复验完成后），对应 v1.1.1。

---

## 0. 现状一句话

**v1.1.1 已发布：复验全部完成（G-A/G-M/G-J + E2E-2，19/20），新缺口 G-O 已修，SPEC 已补录 v1.1.0 修复。剩余只有 T5/T6 两项「不用现在做」。**
...
| 缺口修复 | ✅ 11 + 2 条（v1.1.0）**+ G-O（v1.1.1，复验新发现）** |
| **修复后复验** | ✅ **已做**（2026-09-19，见 ACCEPTANCE.md 复验节） |
| 发布 | ✅ tag `v1.1.1` + [Release](https://github.com/IsANineBug/agent-workflow/releases/tag/v1.1.1)（24.9 KB）+ 本地安装（345/350 行） |
| SPEC 补录 | ✅ 变更记录 + 附录 B 第 12 条（豁免两道重新确认，已留痕） |
...
### T3 · 把 v1.1.0 的修复记进 SPEC ✅ 已完成（2026-09-19）

走的是「用户指令（不用多重验证，一口气办完）+ 显式豁免两道重新确认」路径，已留痕于 §0 变更记录（第三次豁免）。

- 变更记录新增 2026-09-19 22:05 一行；豁免说明从「两次」改为「三次」
- 附录 B 新增第 12 条：13 条缺口与修法、SKILL.md 上限 300→350、复验结果与 G-O
- F-01 验收标准补两句（中场恢复不停手 / 停的是整个任务）；F-12 补 `"source":"test"` 不计入阈值
- 顺带发布了 v1.1.1（tag + Release + 重打包 + 安装点同步）
...
```
T2 ✅ → T1.1 ✅（含 G-O 修复+复跑）→ T1.2 ✅ → T1.3 ✅ → T4 ✅ → T3 ✅ → v1.1.1 已发布 ✅
```

**下一步都是「等条件」的**：T5 evals（等真实运行积累到阈值）、T6 experimental 增量分支（等稳定版真实用起来）、
v1.3 重新设计 §5.6（等豁免数据再攒几次，见附录 B 第 9 条）。
真实使用中若发现问题，照旧流程：记观察 → 攒缺口 → 批量修 → 复验 → 发版。
