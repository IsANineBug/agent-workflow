...
**复验已完成（2026-09-19）：G-A/G-M/G-J 三项复验全部通过 + E2E-2 补测通过（19/20），过程中新发现并修复 G-O。剩 T3。**

| 项 | 状态 |
|---|---|
| PLAN 27 步 | ✅ 全部执行 |
| SPEC / PLAN 两道门禁 | ✅ 已放行并留痕 |
| 20 条 E2E 场景 | ✅ **19 条实测通过**（E2E-20 环境无法模拟，非缺陷） |
| 缺口修复 | ✅ 11 + 2 条（自检新发现）**+ 1 条复验新发现 G-O** |
| **修复后复验** | ✅ **已做**（2026-09-19，G-A/G-M/G-J + E2E-2，见 ACCEPTANCE.md 复验节） |
| 发布 | ✅ tag `v1.1.0` + Release + 本地安装；G-O 修复后 SKILL.md 345 行未打新 tag |
...
### T1 · 复验修复 —— ✅ 已完成（2026-09-19），过程中新发现 G-O 并已修复
...
**结果**：G-M、G-J 一次运行同时复验通过；G-A 第一轮暴露新缺口 **G-O**（「停手」停 skill ≠ 停任务，
agent 在 skill 外即兴实现了整个项目），修复后复跑通过。全部细节见 `ACCEPTANCE.md` 复验节。

---

#### T1.1 · 复验 G-A（§1「用户两个选项都不选」） —— ✅ 复跑通过（2026-09-19）

第一轮**部分失败**：agent 说对了「skill 不适用」、没再「就地补规格」，但把「你看着办吧」当成 skill 外开工的授权，
新建 `index.html` 并覆盖了 `src/main.js` 桩 → **新缺口 G-O**（「停手」的宾语不明，停 skill ≠ 停任务）。

G-O 已修（§1 补「停手停的是整个任务」，提交 `cc0176f`），**复跑通过**：agent 明确拒绝「看着办」授权、
三条合规出口全摆出、场地快照 diff 零差异。详见 `ACCEPTANCE.md` 复验节。
...
### T4 · 补测 E2E-2（父目录不误拒） —— ✅ 已完成（2026-09-19），一次通过

上轮遗留的唯一数据缺口已关闭。prompt 里让子 agent 先 `cd` 并原样回显 `pwd`（避免上轮 G-D 假失败），
agent 先问目录并给选项，未误拒。**E2E 总成绩：19/20 实测通过**（E2E-20 为环境无法模拟，非缺陷）。
场地 `/tmp/v2-parent` 保留。
